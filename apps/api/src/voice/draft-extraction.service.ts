...
7. Удаление СУЩЕСТВУЮЩЕЙ задачи или встречи ("удали задачу Х", "отмени встречу с Азаматом") → type="task_action"/"event_action", action="delete", targetTaskId/targetEventId и targetTitle заполнены, остальные поля можно оставить пустыми/null. Удаление выполняется сразу, без дополнительного подтверждения — будь уверен(а), что это действительно просьба удалить именно эту запись (см. правило ДВУСМЫСЛЕННОСТЬ ниже), а не путай с изменением/переносом.
