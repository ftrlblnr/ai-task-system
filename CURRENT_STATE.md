...
## Assistant Chat (Stage 2, Phases A–G) — текстовый AI-чат
...
стабилизации от 16.09.2026 и генерации файлов от того же дня):
...
- Живой прогон Phase G поймал, что модель иногда дословно копирует
  служебный формат истории (`[shown_task]`/`[shown_event]`/`[file]` с
  `key=value`) в собственный текстовый ответ — инструкция в
...
- Phase H (объединение голосового и текстового путей в один разговор) —
  не начата.
